...
## Run following script in Python to download the rainfall data: 
...
setwd("C:/Users/Gebruiker/Documents/Stage Pipple/510/boundaries")
wshade<-readOGR("districts.shp",layer="districts") 

#working directory for uganda boundary to read kenya boundary

setwd("C:/Users/Gebruiker/Documents/Stage Pipple/510/boundaries")
cliper<-readOGR("uga_admbnda_adm1_UBOS_v2.shp",layer="uga_admbnda_adm1_UBOS_v2")
...
setwd("C:/Users/Gebruiker/Documents/Stage Pipple/510/rainfalldata")

# write data to file 
write.table(arain,file="rainfall",sep="\t",row.names = T,col.names = T)

## Load in rainfall data 
rainfall <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall.txt")


# # Load required packages for loading in rainfall dataset: 
# library(plyr)
# library(utils)
# 
# # Set working directory: 
# setwd("~/Stage Pipple/510/rainfalldata")
# 
# # Load in rainfall data per year: 
# rainfall_2000 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2000.txt")
# rainfall_2001 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2001.txt")
# rainfall_2002 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2002.txt")
# rainfall_2003 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2003.txt")
# rainfall_2004 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2004.txt")
# rainfall_2005 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2005.txt")
# rainfall_2006 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2006.txt")
# rainfall_2007 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2007.txt")
# rainfall_2008 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2008.txt")
# rainfall_2009 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2009.txt")
# rainfall_2010 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2010.txt")
# rainfall_2011 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2011.txt")
# rainfall_2012 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2012.txt")
# rainfall_2013 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2013.txt")
# rainfall_2014 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2014.txt")
# rainfall_2015 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2015.txt")
# rainfall_2016 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2016.txt")
# rainfall_2017 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2017.txt")
# rainfall_2018 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2018.txt")
# rainfall_2019 <- read.delim("~/Stage Pipple/510/rainfalldata/rainfall_2019.txt")
# 
# # Merge all the rainfall data together in one dataframe: 
# rainfall <- join_all(list(rainfall_2000, rainfall_2001, rainfall_2002, rainfall_2003, 
#                           rainfall_2004, rainfall_2005, rainfall_2006, rainfall_2007, 
#                           rainfall_2008, rainfall_2009, rainfall_2010, rainfall_2011, 
#                           rainfall_2012, rainfall_2013, rainfall_2014, rainfall_2015, 
#                           rainfall_2016, rainfall_2017, rainfall_2018, rainfall_2019), by = 'ID')
...
# Set working directory 
setwd("C:/Users/Gebruiker/Documents/Stage Pipple/510/desinventar")

# Load in Desinventar Uganda dataset. Downloaded this file from: 
# https://www.desinventar.net/DesInventar/download_base.jsp?countrycode=uga
DI_uga <- read_csv("DI_uga.csv")
...
# Set working directory 
setwd("C:/Users/Gebruiker/Documents/Stage Pipple/510/CRA")

#Load in Community Risk Assessment dataset. Downloaded this file from: 
# https://dashboard.510.global/#!/community_risk
CRA <- read_excel("CRA Oeganda.xlsx")

#-----------------Prepare the rainfall dataset for merging-------------------------------- 

# Load required packages for preparing rainfall dataset: 
library(dplyr)
library(rgdal)
...
# Working directory for uganda boundary to read districts (downloaded those 
# boundaries from: https://www.510.global/community-risk-assessment/): 
setwd("C:/Users/Gebruiker/Documents/Stage Pipple/510/boundaries")
wshade <- readOGR("districts.shp",layer="districts") 
...
# Made some extra rainfall predictors: 
...
#--------------------Prepare the Desinventar dataset for merging---------------------------

# Load required packages for preparing Desinventar dataset:  
library(zoo)

# Select only the rows with 'flood': 
...
# Put the year, month and day of the flood together with dash between them and call this date: 
...
DI_uga[364,23] <- as.Date("2013-03-01") # geen data 
DI_uga[366,23] <- as.Date("2014-04-01") # geen data  
DI_uga[373,23] <- as.Date("2007-07-01") # geen data
DI_uga[375,23] <- as.Date("2012-01-01") # geen data
DI_uga[376,23] <- as.Date("2012-01-01") # geen data
DI_uga[377:385,23] <- as.Date("2012-04-01") # geen data
DI_uga[386:387,23] <- as.Date("2012-06-01") # geen data
DI_uga[388,23] <- as.Date("2013-04-01") # geen data
DI_uga[389:395,23] <- as.Date("2013-06-01") # geen data
DI_uga[399:401,23] <- as.Date("2010-01-01") # geen data
DI_uga[414,23] <- as.Date("2014-09-01") # geen data
DI_uga[454,23] <- as.Date("2012-08-09")
DI_uga[455:457,23] <- as.Date("2012-09-02")
DI_uga[459,23] <- as.Date("2014-03-12") #deze heeft 15e maand
...
DI_uga[964,23] <- as.Date("2011-02-01") # geen data 
...
DI_uga[1080:1081,23] <- as.Date("2010-01-01") # geen data 
DI_uga[1090:1096,23] <- as.Date("2012-05-01") # geen data
DI_uga[1101,23] <- as.Date("2013-05-01") # geen data 
...
# # Deze code gebruikt om de goede datums in te vullen op basis van de rainfall data: 
# test <- rainfall %>% filter(district == "ABIM", date >= "2011-03-01", date <= "2011-05-30")
# which.max(test$rainfall)
# View(test)

#--------------------Prepare the CRA dataset for merging -----------------------

# Load required packages for preparing CRA dataset: 
...
# Write data to a file so not every time all the preprocessing steps have to be made: 
setwd("~/Stage Pipple/510/")
write.table(data,file="mergeddataset.txt",sep="\t",row.names = T,col.names = T)

#--------------------------- Load in merged dataset ----------------------------

setwd("~/Stage Pipple/510/")
data <- read.delim("~/Stage Pipple/510/mergeddataset.txt")
...
# Make plot of the differences: 
# data$difference1 <- data$difference
# data$difference1[data$difference1 < 0] <- NA
# data$difference1[data$difference1 == 0] <- NA

# library(ggplot2)
# hist(data$difference1, xlim = c(0, 50), breaks=seq(0,5000,1))
# table(data$difference1)

data <- dplyr::select(data, "pcode", everything())

#remove all variables which I don't need to aggregate and don't need mean off: 
...
setwd("~/Stage Pipple/510/")
data_agg <- read.delim("~/Stage Pipple/510/aggregateddataset.txt")
...
# library(corrplot)
# correlations <- round(cor(data_agg_dep[,4:26]),2)
# corrplot(correlations)

# As the correlations between the dependents were not high, except all the binary variables (and the correlation between impact and 
# rainfall was higher when I only took the binary variables into account)
# I decided to only take the binary variables into account when creating the impact variable 
# The data quality of the binary variables seems better 
# When there is a 1 on a binary variable we assume that the risk of e.g. a people death/injured is also higher. 

##### Take rowsums of values on all dependent variables to make one total affect variable:
...
nrow(data_agg_dep[data_agg_dep$DEP_total_affect_binary == 0,])
nrow(data_agg_dep[data_agg_dep$DEP_total_affect_binary == 1,])
...
# Do mean imputation for every column that has a missing value:
...
# # Variables with very little unique values: 
...
# Remove variables which very little unique values (and therefore have a weird cdplot)
...
# Remove rainfall variables as they don't have any additional value: 
...
# correlations <- round(cor(data[,c(10:32)]),2)
# corrplot(correlations)
# View(correlations)
...
data <- dplyr::select(data, -c(CRA_general_land_area, CRA_vulnerability_employed, CRA_coping_educational_facilities,
                      ...
                      CRA_general_coping, CRA_general_hazard))

data <- dplyr::select(data, -c(CRA_vulnerability_poverty, CRA_coping_sanitation))

# Standardize the variables for lasso regression and to make it more interpretable:
data[,5:21] <- scale(data[,5:21])

# # Add rainseasons as extra independent variable:
# data$monthday <- as.numeric(strftime(data$GEN_date, "%m%d"))
# data$rainyseason[data$monthday >= 215 & data$monthday < 600] <- 1
# data$rainyseason[data$monthday >= 915 & data$monthday < 1131] <- 1
# data$rainyseason[is.na(data$rainyseason)] <- 0
# data$rainyseason <- as.factor(data$rainyseason)

# # General information about the data:  
# summary(data)
# str(data)

# # Plot some distributions to get more feeling of the data: 
# library(plyr)
# library(psych)
# multi.hist(data[,sapply(data, is.numeric)])
# 
# # Plot all the boxplots of the numeric variables: 
# for (i in 5:length(data)) {
#   boxplot(data[,i], main=names(data[i]), type = "l")
# }

# ---------------- Logistic regression with lasso: total affect ----------------
...
nfolds <- 5 #change number of folds
set.seed(1)
...
p.value.lasso <- matrix(NA, nfolds)
coefs.lasso <- matrix(NA, 18, nfolds) #number of numeric independent variables (no date + district) + 1 (= intercept) 
cm.lasso <- matrix(NA, 4, 5)

                                 
set.seed(1)
#nested for-loop for 10-fold crossvalidation to find the optimal k
for(i in 1:nfolds) {
  train <- folds != i
  train <- data[train,]
  test <- folds == i
  test <- data[test,]
  xtrain <- as.matrix(train[,5:21]) #columns of x variables 
  ytrain <- as.matrix(train[,1]) #column of y variable 
  xtest <- as.matrix(test[,5:21])
  ytest <- as.matrix(test[,1])
  # 10-fold cross-validation to determine optimal lambda: 
  # set.seed(1)
  cv.lasso <- cv.glmnet(xtrain, ytrain, alpha = 1, family="binomial") #to do ridge regression, set alpha = 1 to alpha = 0
  betas <- coef(cv.lasso, cv.lasso$lambda.min)
  betas <- as.matrix(betas)
  coefs.lasso[,i] <- betas
  # Determine misclassification error rate for the model with optimal lambda:
  lasso.predict.pr <- predict(cv.lasso, newx=xtest, s=cv.lasso$lambda.min, type = "response") #this gives the predicted probabilities
  pred <- prediction(lasso.predict.pr, test$DEP_total_affect_binary) #change dependent variable
  ...
  p.value.lasso[i] <- confusionmatrix.lasso[["overall"]][["AccuracyPValue"]]
...
apply(p.value.lasso, 2, mean)

rowMeans(cm.lasso)

# -------------------- Support Vector Machine: total affect --------------------
...
for(i in 1:nfolds) {
  ...
  test <- test[,c(1, 5:21)]
  # training with grid to determine optimal tuning parameters: 
  #grid <- expand.grid(sigma = c(.25, .50, 1, 2, 4), C = c(.001, .01, .1, 1, 5, 10, 100))
  tune.out <- train(DEP_total_affect_binary ~ ., data = train, method = "svmRadial") #, tuneGrid = grid) #change dependent variable
  # http://topepo.github.io/caret/train-models-by-tag.html#Support_Vector_Machines which method to use? 
  importance <- varImp(tune.out, scale = FALSE)$importance[,2]
  importance <- as.matrix(importance)
  coefs.svm[,i] <- importance 
  # Determine misclassification error rate for the model with optimal tuning parameters:
  model.svm <- ksvm(DEP_total_affect_binary~.,data=train,kernel="rbfdot", #change dependent variable. 
                    kpar=list(sigma=tune.out$bestTune[,1]),C=tune.out$bestTune[,2], prob.model=TRUE)
  ...
  #AUC
  pred <- prediction(svm.predict, test$DEP_total_affect_binary) #change dependent variable. 
  ...
  p.value.svm[i] <- confusionmatrix.svm[["overall"]][["AccuracyPValue"]]
...
apply(p.value.svm, 2, mean)

rowMeans(cm.svm)

# -------------------------- Random Forest: total affect -----------------------
...
p.value.rf <- matrix(NA, nfolds)
coefs.rf <- matrix(NA, 17, nfolds) #number of numeric independent variables (no date + district)
cm.rf <- matrix(NA, 4, 5)
...
for(i in 1:nfolds) {
  ...
  train <- train[,c(1, 5:21)]#first number is dependent variable and the others independent variables 
  test <- test[,c(1, 5:21)]
  #select mtry and ntrees: 
  #mtry <- tuneMTRY(train1, iterations = 2, mtry_length = 30, graph = TRUE)
  #fit model on traindata: 
  ran.for <- randomForest(DEP_total_affect_binary ~., data = train, importance = TRUE, ntree = 1000) #, mtry = which.min(mtry$oob)) #change dependent variable 
  #variable importance: 
  importance <- importance(ran.for)[,3]
  importance <- matrix(importance)
  coefs.rf[,i] <- importance
  # Determine misclassification error rate for the model: 
  ran.for.pr <- predict(ran.for, newdata = test, type = "prob")
  ran.for.pr <- as.numeric(ran.for.pr[,2])
  pred <- prediction(ran.for.pr, test$DEP_total_affect_binary) #change dependent variable  
  auc <- performance(pred, measure='auc')
  ...
  p.value.rf[i] <- confusionmatrix.ran.for[["overall"]][["AccuracyPValue"]]
  # plot confusion matrix: 
...
apply(p.value.rf, 2, mean)

rowMeans(cm.rf)

# -------------------------- Stepwise logistic regression: total affect -----------------------
...
for(i in 1:nfolds) {
  ...
  pred <- prediction(step.lr.pr, test$DEP_total_affect_binary) #change dependent variable  
  auc <- performance(pred, measure='auc')
  ...
  # confusionmatrix.step.lr <- confusionMatrix(as.factor(step.lr.pr), as.factor(ytest))
  # #accuracy 
  # accuracy.lasso[i] <- confusionmatrix.lasso[["overall"]][["Accuracy"]]
  # #f1 score 
  # F1.lasso[i] <- F1_Score(test$DEP_total_affect_binary, lasso.predict.pr) 
} 
...
# -------------------------- Logistic regression: total affect -----------------------
...
for(i in 1:nfolds) {
  ...
  test <- folds == i
  test <- data[test,]
  train <- train[,c(1, 5:23)]#first number is dependent variable and the others independent variables 
  test <- test[,c(1, 5:23)]
  lr <- glm(DEP_total_affect_binary ~ . -CRA_coping_electricity -CRA_vulnerability_poverty, data = train, family =  "binomial") 
  lr.pr <- predict(lr, newdata = test, type = "response")
  pred <- prediction(lr.pr, test$DEP_total_affect_binary) #change dependent variable  
  auc <- performance(pred, measure='auc')
  AUC.lr[i] <- auc@y.values[[1]]
  perf <- performance(pred, measure = "tpr", x.measure = "fpr") 
  plot(perf, col=rainbow(7), main="ROC curve", xlab="1-Specificity", 
       ylab="Sensitivity")    
  abline(0, 1) #add a 45 degree line
...
#---------------------Logistic regresion with lasso: with CI---------------------

# Lasso logistic regression: 
nfolds <- 2 #change number of folds
set.seed(1)
folds <- sample(rep(1:nfolds, length=nrow(data), nrow(data)))
table(folds)
AUC <- matrix(NA, nfolds)
AUC_CI <- matrix(NA, nfolds, 3)
coefs.lasso <- matrix(NA, 42, nfolds) #number of numeric independent variables (no date + district) + 1 (= intercept) 
...
for(i in 1:nfolds) {
  ...
  test <- folds == i
  test <- data[test,]
  xtrain <- as.matrix(train[,5:45]) #columns of x variables 
  ytrain <- as.matrix(train[,1]) #column of y variable 
  xtest <- as.matrix(test[,5:45])
  ytest <- as.matrix(test[,1])
  # 10-fold cross-validation to determine optimal lambda: 
  cv.lasso <- cv.glmnet(xtrain, ytrain, family="binomial")
  betas <- coef(cv.lasso, cv.lasso$lambda.min)
  betas <- as.matrix(betas)
  coefs.lasso[,i] <- betas
  # Determine misclassification error rate for the model with optimal lambda:
  lasso.predict.pr <- predict(cv.lasso, newx=xtest, s=cv.lasso$lambda.min, type = "response") #this gives the predicted probabilities
  pred <- prediction(lasso.predict.pr, test$DEP_total_affect_binary) #change dependent variable
  pROC_obj <- roc(test$DEP_total_affect_binary, as.numeric(lasso.predict.pr), ci = TRUE, plot = TRUE, print.auc = TRUE)
  AUC[i] <- pROC_obj[["auc"]]
  AUC_CI[i, 1] <- pROC_obj[["ci"]][1]
  AUC_CI[i, 2] <- pROC_obj[["ci"]][2]
  AUC_CI[i, 3] <- pROC_obj[["ci"]][3]
...
#-------------------- Support Vector Machine: with CI --------------------
...
AUC <- matrix(NA, nfolds)
AUC_CI <- matrix(NA, nfolds, 3)
coefs.svm <- matrix(NA, 40, nfolds) #number of numeric independent variables (no date + district)
...
for(i in 1:nfolds) {
  ...
  model.svm <- ksvm(DEP_total_affect_binary~.,data=train,kernel="rbfdot", #change dependent variable. 
                    kpar=list(sigma=tune.out$bestTune[,1]),C=tune.out$bestTune[,2], prob.model=TRUE)
  svm.predict <- predict(model.svm, test, type="probabilities")
  pred <- prediction(as.numeric(svm.predict[,2]), test$DEP_total_affect_binary) #change dependent variable. 
  pROC_obj <- roc(test$DEP_total_affect_binary, as.numeric(svm.predict[,2]), ci = TRUE, plot = TRUE, print.auc = TRUE)
  AUC[i] <- pROC_obj[["auc"]]
  AUC_CI[i, 1] <- pROC_obj[["ci"]][1]
  AUC_CI[i, 2] <- pROC_obj[["ci"]][2]
  AUC_CI[i, 3] <- pROC_obj[["ci"]][3]
...
AUC <- matrix(NA, nfolds)
AUC_CI <- matrix(NA, nfolds, 3)
coefs.rf <- matrix(NA, 40, nfolds) #number of numeric independent variables (no date + district)
...
for(i in 1:nfolds) {
  ...
  test <- folds == 1
  test <- data[test,]
  #train1 <- train[,c(1, 2, 5:44)]
  train <- train[,c(1, 4:43)]#first number is dependent variable and the others independent variables 
  test <- test[,c(1, 4:43)]
  #select mtry and ntrees: 
  #mtry <- tuneMTRY(train1, iterations = 1, mtry_length = 30, graph = TRUE)
  #fit model on traindata: 
  ran.for <- randomForest(DEP_total_affect_binary ~., data = train, importance = TRUE) #, mtry = which.min(mtry$oob)) #change dependent variable 
  #variable importance: 
  importance <- importance(ran.for)[,3]
  importance <- matrix(importance)
  coefs.rf[,i] <- importance
  # Determine misclassification error rate for the model: 
  ran.for.pr <- predict(ran.for, newdata = test, type = "prob")
  pred <- prediction(as.numeric(ran.for.pr[,2]), test$DEP_total_affect_binary) #change dependent variable  
  pROC_obj <- roc(test$DEP_total_affect_binary, as.numeric(ran.for.pr[,2]), ci = TRUE, plot = TRUE, print.auc = TRUE)
  AUC[i] <- pROC_obj[["auc"]]
  AUC_CI[i, 1] <- pROC_obj[["ci"]][1]
  AUC_CI[i, 2] <- pROC_obj[["ci"]][2]
  AUC_CI[i, 3] <- pROC_obj[["ci"]][3]
