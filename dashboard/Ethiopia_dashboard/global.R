...
ethiopia_impact <- read_delim("data/Eth_impact_data2.csv", ";", escape_double = FALSE, trim_ws = TRUE)
...
eth_admin3 <- eth_admin3 %>%
  left_join(floods_per_wereda %>% dplyr::select(pcode, n_floods), by = c("WOR_P_CODE" = "pcode")) %>% dplyr::filter(!is.na(n_floods))
